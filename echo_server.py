...
"""EchoServer
При запуске сервер начинает слушать заданный TCP-порт. При подключении клиента
следует принять соединение и затем в цикле выполнять следующую
последовательность:
- считать из сокета полученные данные
- отправить их обратно в неизменном виде
Продолжаем этот цикл до самостоятельного отключения клиента или получения
команды "disconnect\n".
При получении команды "disconnect\n" сервер должен закрыть клиентский сокет.
Проверить корректную работу сервера при помощи команды telnet. Проверить, что
сервер успешно обслуживает более одного одновременного соединения."""

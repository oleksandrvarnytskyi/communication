...
"""HttpServer
Консольный HTTP-сервер. После запуска обрабатывает в бесконечном цикле
соединения. Отвечает на GET-запросы, на запросы с остальными методами отвечает
с кодом '405 Method Not Allowed'. Если файл, соответствующий GET-запросу, не
найден, то '404 Not Found'. Завершается сервер нажатием комбинации Ctrl+C.
На каждый запрос выводить в консоль цифровой код ответа и URL.
Адрес, порт и корневой каталог HTTP-сервера определяется конфигурацией в файле
http.conf (в папке с исполняемым файлом сервера)"""
